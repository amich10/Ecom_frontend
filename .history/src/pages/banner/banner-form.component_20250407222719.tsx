...
const BannerForm = ({submitEvent,banner}: IformProps) =>{



    const bannerValidaterDTo =yup.object({
    ...
    const {handleSubmit,control,formState:{errors,isSubmitting},setValue,reset} = useForm({
        ...
        resolver: yupResolver(bannerValidaterDTo),
        // resetOptions:{
        //     keepValues:false,
        //     keepErrors:false,
    ...
    useEffect(() =>{
       ...
       if(banner){ //removes undefined
        setValue('title',banner?.title)
        setValue('status',banner?.status),
        // setValue('image',banner?.image) //image yesari mildaina
        setThumbnail(banner?.image?.optimizedUrl)
        ...
        <form onSubmit={handleSubmit(submitEvent)} onReset={() =>{
            ...
            <div className="flex flex-col gap-3">
                ...
                <div className=" flex">
                    ...
                    setValue={(name:string, file:any) => setValue(name as keyof IBannerData, file)}/>
                    </div>
                </div>
