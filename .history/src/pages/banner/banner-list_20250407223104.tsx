...
const BannerList = () => {
  ...
  const getAllbanners = async ({
    ...
    try {
      ...
      notifyUserRegistration(
        "banners cannot be displayed at this time. Please try again later.",
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      // debounce: user ley input xodeko time : background resource
      getAllbanners({
        page: pagination.current,
        limit: pagination.pageSize,
        search: search,
  ...
  const columns = [
    ...
    },
    {
      title: "Image",
      dataIndex: "image",
      render: (value: { url: string; optimizedUrl: string }) => (
        <img src={value.optimizedUrl} alt="" className="w-25" />
      ),
    },
    {
      title: "Status",
          ...
          <>
            <div className="flex gap-5">
              ...
              <Popconfirm
                ...
                okText="Yes"
                cancelText="Cancel"
                okButtonProps={{ className: "bg-red-800!" }}
                cancelButtonProps={{ className: "bg-green-700! text-white!" }}
                onConfirm={async () => {
                  try {
                    ...
                    notifyUserRegistration(
                      "banner cannot be deleted at this time. Please try again",
                      NotificationType.ERROR
                    );
                  }
                }}
  ...
  return (
    <Layout.Content className="rounded-md p-5">
      <div className="border-b border-b-gray-500">
        <Typography.Title className="text-center text-white! bg-yellow-500 text-3xl p-2 rounded-md">
          banners
        </Typography.Title>
      </div>
      <div className="flex justify-between mt-3">
        ...
        <NavLink
          ...
          className="bg-green-600! text-white! rounded-md p-1.5 hover:bg-green-800!"
        >
