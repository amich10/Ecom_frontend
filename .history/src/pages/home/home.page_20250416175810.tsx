...
const HomePage = () => {
  ...
  const submitLogin = async (data: ICredentials) => {
    try {
      ...
      notifyUserRegistration("Welcome back!", NotificationType.SUCCESS);
      navigate("/" + user.role);
    } catch (exception) {
      notifyUserRegistration("sorry cannot login at this  moment", NotificationType.ERROR);
    }
  };
   // if already loggedin
   useEffect(() => {
  ...
  return (
    <div className="h-screen flex justify-center items-center">
      <div className="bg-white shadow-2xl shadow-violet-400 border-2 border-violet-600 w-120 rounded-md p-4 font-serif">
        ...
        <div className="mt-3
        ">
          <SubmitButton isSubmitting={isSubmitting} classes={"bg-violet-700! w-full font-bold h-9! text-[17px]!"}>Login</SubmitButton>
        </div>
