        ...
        <Content>
            ...
            <div className="w-1/4 h-full border-2 border-gray-300 rounded-md shadow-md bg-white">
                ...
                {["User 1", "User 2"].map((user, index) => (
                    <div key={index} className="overflow-y-auto h-[calc(100%-120px)] flex gap-3 p-2">
                        <img src="https://placehold.co/50x50" alt={`${user} Avatar`} className="rounded-full w-10 h-10"/>
                        <div className="flex flex-col">
                            <h1 className="font-bold text-blue-600 text-sm">{user}</h1>
                            <p className="text-xs text-gray-500 italic">This is a message preview from {user}</p>
                        </div>                 
                    </div>
                ))}
            </div>
