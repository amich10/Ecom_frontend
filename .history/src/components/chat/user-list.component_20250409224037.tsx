...
const UserList = () => {
  ...
  const currentUser = useSelector((root:RootState) =>{ //when specific user is clicked
    return root.user.userDetail as ILoggedInUserDatao
  })
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-4">
        {userList.length > 0 ? (
          userList.map((user: any, index: number) => (
            <div
              key={user._id || index}
              className={`flex gap-3 items-start p-2 rounded-md hover:bg-gray-200 transition ${currentUser && currentUser.id === user._id} ? bg-blue-400` }
